...
public class Translator {
    ...
    public void prog() {        
        switch(look.tag) {
            ...
            //code.emitLabel(lnext_prog); //Non necessario -> L1
            match(Tag.EOF);
            try {
                code.toJasmin();
            }
    ...
    private void stat(int lnext_stat) {
        ...
        switch(look.tag) {
            ...
            case Tag.ASSIGN:
                ...
                match(Tag.TO);
                idlist(lnext_stat, 4);
                //code.emit(OpCode.GOto, lnext_stat); Non necessario
                break;
        ...
        case Tag.ID:
            int id_addr = st.lookupAddress(((Word)look).lexeme);
                ...
                if (num == 4) {
                    ...
                    match(Tag.ID);
                    idlistp(lnext_idlist, num, id_addr);
    ...
    private void idlistp(int lnext_idlistp, int num, int old_addr) {
        switch(look.tag) {
            case ',':
                match(',');
                int id_addr = st.lookupAddress(((Word)look).lexeme);
                ...
                if (num == 4) {
                    code.emit(OpCode.iload, old_addr);
                    //code.emit(OpCode.dup);
                    code.emit(OpCode.istore, id_addr);
                    match(Tag.ID);
                    idlistp(lnext_idlistp, num, id_addr);
                } else {
                    code.emit(OpCode.invokestatic, 0);
                    code.emit(OpCode.istore, id_addr);
                    match(Tag.ID);
