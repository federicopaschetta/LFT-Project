...
public class Translator {
    ...
    public void prog() {        
        switch(look.tag) {
            ...
            statlist(lnext_prog);
            //code.emit(OpCode.GOto, lnext_prog); Non necessario
            //code.emitLabel(lnext_prog); Non necessario
            match(Tag.EOF);
            try {
                code.toJasmin();
            }
    ...
    private void stat(int lnext_stat) {
        ...
        switch(look.tag) {
            ...
            case Tag.WHILE:
                ...
                bexprlist(lnext_true, lnext_false);
    ...
    private void bexprlist(int ltrue, int lfalse) {
        int bexprlist_label;
        if(look.tag == '!') {
            match('!');
                bexprlist_label = ltrue;
                ltrue = lfalse;
                lfalse = bexprlist_label;
                bexprlist(ltrue, lfalse);
            ...
            case "&&":
                match(Tag.AND);
                bexprlist_label = code.newLabel();
                bexprlist(bexprlist_label, lfalse);
                code.emitLabel(bexprlist_label);
                bexprlist(ltrue, lfalse);
                break;
            ...
            case "||":
                match(Tag.OR);
                bexprlist_label = code.newLabel();
                bexprlist(ltrue, bexprlist_label);
                code.emit(OpCode.GOto, ltrue);
                code.emitLabel(bexprlist_label);
                bexprlist(ltrue, lfalse);
                break;
